...
const Navbar = ({ userInitial, onLogout }) => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
        ? "py-4 bg-[#1a103d]/80 backdrop-blur-xl border-b border-white/10 shadow-lg"
        : userInitial ? "py-4 bg-[#1a103d] border-b border-white/10" : "py-6 bg-transparent"
        }`}
    >
        ...
        <div className="flex items-center gap-4">
          {userInitial ? (
            <div className="flex items-center gap-4">
              ...
              <div className="w-10 h-10 bg-indigo-600 border-2 border-white/20 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg">
                {userInitial}
              </div>
            </div>
          ) : (
